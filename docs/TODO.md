...
### 1. 交易頁日曆檢視（新分支：`feature/calendar-view`）
**目標：** 交易紀錄頁新增日曆顯示模式，點擊日期可直接在當天新增記帳。
- [ ] 前端：表格上方加「列表 / 日曆」切換 Tab
- [ ] 前端：日曆視圖顯示每日收支小計（點擊日期展開當日明細）
- [ ] 前端：點擊日期自動帶入日期欄位開啟新增交易表單
- [ ] 後端：確認 `GET /api/transactions` 的日期篩選支援月份查詢
...
### 1. CSV 匯入功能（新分支：`feature/csv-import`）
**目標：** 讓使用者上傳 CSV 檔案，將歷史交易記錄批次匯入系統。
- [ ] 定義 CSV 格式規範（日期、金額、類別、帳戶、備註等欄位）
- [ ] 後端：`POST /api/transactions/import`，接收 CSV、解析、驗證、批次寫入
- [ ] 後端：回傳匯入結果（成功筆數、失敗筆數 + 錯誤原因）
- [ ] 前端：上傳區塊 + 格式範本下載 + 匯入結果摘要顯示
- [ ] 考慮：可支援匯出（`GET /api/transactions/export`）讓使用者備份

### 2. 隱藏金額（新分支：`feature/privacy-mode`）
**目標：** 全站加入隱私模式，點擊眼睛按鈕將所有金額數字替換成 `***`，方便在公開場合使用。
- [ ] 前端：Zustand store 加入 `privacyMode` 全域狀態，持久化到 localStorage
- [ ] 前端：Header 加入眼睛圖示按鈕（`EyeOutlined` / `EyeInvisibleOutlined`）
- [ ] 前端：建立 `usePrivacyAmount(value)` hook，privacyMode 開啟時回傳 `***`
- [ ] 前端：套用到所有顯示金額的地方（Dashboard、TransactionList、AssetList、帳戶餘額）
